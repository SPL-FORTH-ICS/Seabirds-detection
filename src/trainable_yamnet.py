...
def get_trainable_yamnet(load_weights = True, input_duration = None, num_classes = 2):
  ...
  if load_weights:
    ...
    #yamnet_pretrained.load_weights('./retrained_Yamnet.hdf5')    
...
#%%%% NIKOS STEFANAKIS 
def get_retrained_yamnet(pathIN, input_duration = None, num_classes = 2, activation_layer='softmax'):
  params = Params_class() #initialize parameters with the ones hardcoded in params.py
  params.patch_window_seconds = input_duration
  params.patch_hop_seconds = input_duration
  # params.classifier_activation = 'softmax'   
  params.classifier_activation =  activation_layer  
  params.num_classes = num_classes  
  yamnet_retrained = get_batch_model(params) #model defined
  yamnet_retrained.load_weights(pathIN)    
  
  return yamnet_retrained
